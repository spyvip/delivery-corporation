...
function final_calculate(){
	var final_total=0;
       //var r_subtotal=$("#subtotal").text();
			...
			var tip= $(".dpui-numberPicker-input").val().replace("$", "");
	 ...
	 $('#select_rest').show();
					...
					var html = '';		
						...
						var len=value.length;				
								for(i=0; i < len; i++)
								{
							html += '<tr id="tryme_rest" class="btn spaceUnder_rest" valuename="'+value[i]['name']+'" valueid="'+value[i]['id']+
			'" style="float:left;width:450px;white-space:pre-line;text-align:left;border: 1px solid #c4c4c4;margin-left:15px;">'+
							'<td style="margin-right:10px"><label><img style="width:70px;height:70px;margin-right:15px;" src= "/delivery/resources/images/'+value[i]['image']+'"/></label></td>'+
...
//alert(last2);
	...
	if(new_time_hr<12)
	{
		//alert("New time:"+new_time_hr+":"+min+am);
	...
	//alert("New time:"+new_time_hr+":"+min+pm);
...
/*
		...
		things.rest_thing.push({"id":i_restid ,"name":i_restname});
  ...
  for(var i=0; i < things.rest_thing.length; i++){
	   $("#cart_item_details").append('<tr class="count_restaurant" restid='+ things.rest_thing[i].id+' name='+things.rest_thing[i].name+' style="display: block;margin-bottom:10px;"><td number="'+i+'" id='+things.rest_thing[i].name+' style="width:700px"><b>'+things.rest_thing[i].name+'</b></td></tr>');
			 ...
			 table.find('tr').each(function (i,e) {
			//var q=$(this).closest('td').attr('number');
...
/*
        ...
        var $tds = $(this).find('td'),
		     ...
		     i_restid = $tds.eq(9).text();
			    ...
			    thelist.push({"id":i_restid ,"name":i_restname}); 
			});
	...
	function load_corporate_center(){
		...
		$.ajax({
			...
			type: "GET",
			data: {"corpZipCode": "19426"},
			success: function(data) {	   
				$.each(data, function(key, value) { 
					var tblRow2 = '<li><a style="cursor:pointer" id="'+value['id']+'" onclick="myFunction()">'+value['name']+'</a></li>'
					$("#dd3").append(tblRow2);   
				});		
			}, 
			error: function() {
      		...
      		success: function(data){
				...
				//$("#backto_rest").html('<a href="http://localhost:8080/delivery/rests/restaurants?zipcode=19425&orderType=CORP">&nbsp;&nbsp;<i class="left"></i><b >'+select_rest+'</b></a>');
						...
						$.each(data, function(key, value) { 
							...
							'ordertype="'+ordertype+'" value=' + value['id']+' restaurantBustypeId="'+value['restaurantBustypeId']+'" style="border:1px solid #c4c4c4;height:100px;width:900px;padding-left:5px;">'+
...
// table_rowData1 = $("<td style='display: none'>"+section_edit+"</td>");
	...
	for(i=1;i<=6;i++){
		...
		if((l=='Sat')||(l=='Sun')){
			...
			document.getElementById("demo"+i).innerHTML = l+" "+m+" "+n;
		}
	}
...
final_calculate();					
	...
	$("#remove_coupon").on('click', function(e) {
		$("#review_coupon").html('<a data-toggle="modal" data-target="#myModal_coupon">Apply Coupon</a>');
	...
	else if( $('#disply_new_card').is(':empty') ) 
	{
		alert("Please select card to proceed");
			...
			var suite_number=$("#suite_number").text();
			//
			...
			var driver_tip = $(".dpui-numberPicker-input").val().replace("$", "");
		...
		//	alert(rest_table);
			...
			$('#cart_item_details .count_restaurant').each(function(){  
				...
				the_rest.push({restaurantId:restid,	name:restname });
			});
			...
			for(j=0;j<the_rest.length;j++){
				...
				$(table).each(function(){  
					...
					if(i_section1==""){
						...
						the_section.push({id : i_section_id1, name :i_section_heading1, value : i_value1});
					}
					...
					if(i_section2==""){
						...
						the_section.push({id : i_section_id2, name :i_section_heading2, value : i_value2});
					}
					...
					if(i_section3==""){
						...
						for(var k = 0; k < myarray3.length; k++)
						{
						 ...
						 the_section.push({id : i_section_id3,
					...
					if(i_section4==""){
						...
						the_section.push({id : i_section_id4, name :i_section_heading4, value : i_value4});
					}
					...
					if(i_section5==""){
						...
						the_section.push({id : i_section_id5, name :i_section_heading5, value : i_value5});
					}
					...
					if(i_section6==""){
						...
						the_section.push({id : i_section_id6, name :i_section_heading6, value : i_value6});
					}
					
					console.log("the_section "+the_section);
					
					the_menu.push({	id:i_menuid,
									name:i_product,
				...
				listOfObjects.push({
					"name": the_rest[j].name, "restaurantId": the_rest[j].restaurantId, 
					"menus": the_menu,
